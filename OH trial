...
def OH_calculation():
    """
    -This function finds which overhead is the highest amongst all of them, and writes the category name in the txt file
    - This function has no parameters
    """
    # Setting original values for variables as local variables
    day_counter= 0
    NP_new = 0
    NP_high = 0
    # Setting a while loop which ends when all items in the overheads file are covered
    while day_counter  < len(overheads):
        # Creating variable capturing the Overheads' values which changes when the day_counter variable changes
        ...
        if NP_new > NP_high:
            ...
            # using mode = "w" to write data in file
